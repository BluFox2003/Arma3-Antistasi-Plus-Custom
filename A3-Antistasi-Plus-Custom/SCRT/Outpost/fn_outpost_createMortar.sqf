...
private _textX = format ["%1 Mortar Emplacement", nameTeamPlayer];

private _marker = createMarker [format ["FIAMortarpost%1", random 1000], _position];
...
[[teamPlayer,civilian],_taskId,["We are sending a team to establish a mortar emplacement. Use HC to send the team to their destination","Mortar Emplacement Deploy",_marker],_position,false,0,true,"Move",true] call BIS_fnc_taskCreate;
[_taskId, "outpostTask", "CREATED"] remoteExecCall ["A3A_fnc_taskUpdate", 2];

_formatX = [SDKSL,SDKMG,SDKMil,SDKMil,SDKMedic];
...
if ({(alive _x) and (_x distance _position < 10)} count units _groupX > 0) then {
	...
	mortarpostsFIA = mortarpostsFIA + [_marker]; publicVariable "mortarpostsFIA";
